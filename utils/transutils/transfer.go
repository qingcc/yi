...
func getSign(query string, salt string) string {
	return utils.Strmd5(appId + query + salt + secryptkey)
}
...
func SendTransData(req TransRequest, res interface{}, storage bool) (httpMessageResult commobj.HttpMessageResult) {
	...
	request_url, reqData, err := newRequest(req)
	if err != nil {
	...
	code, message, data := httpService.SendJson(request_url, reqData, header, 1, res)
	httpMessageResult.ResultCode = code
	httpMessageResult.SplResultCode = code
	httpMessageResult.Url = request_url
...
func newRequest(req TransRequest) (request_url string, data []byte, err error) {
	if req.Ssl {
		request_url = Urls
	}else {
		request_url = Url
	...
	param := url.Values{
		...
		"salt":  {req.Salt},                                            //		INT	  必填	随机数
		"sign":  {getSign(req.Query, req.Salt)}, //		TEXT  必填	签名	appid+q+salt+密钥 的MD5值
	}
