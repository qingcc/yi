...
import (
	"fmt"
	"github.com/qingcc/yi/utils/zkutils"
	"github.com/samuel/go-zookeeper/zk"
	"log"
	"time"
)

func test_conn(path string)  {
	conn := zkutils.GetConn()
	str,err := conn.Create(path, nil)
	log.Println("create:", str, err)
	time.Sleep(time.Minute)
}

func get_all_node(path string)  {
	conn := zkutils.GetConn()
	children, _, err := conn.Children(path)
	if err != nil {
		fmt.Println("get children node failed:", err)
	}
	fmt.Printf("%f \n", children)
	time.Sleep(time.Minute)
}
...
func add()  {
	node1 := &zkutils.ServiceNode{
		Name: "user",
		Host: "127.0.0.1",
		Port: 81,
	}
	node2 := &zkutils.ServiceNode{
		Name: "user",
		Host: "127.0.0.2",
		Port: 82,
	}
	node3 := &zkutils.ServiceNode{
		Name: "/test/user",
		Host: "127.0.0.3",
		Port: 83,
	}
	c := zkutils.NewConn().SetZKList([]string{"47.112.210.86:6035"}).SetZkroot("").Set()
	if err := zkutils.Register(node1); err != nil {
		log.Println("register node1 failed:", err.Error())
	}
	if err := zkutils.Register(node2); err != nil {
		log.Println("register node2 failed:", err.Error())
	}
	if err := zkutils.Register(node3); err != nil {
		log.Println("register node3 failed:", err.Error())
	}
}
func get()  {
	ids,_, err := zkutils.GetConn().Children("/test")
	if err != nil {
